...
@section('page_title')
    <div class="app-title">
        <div>
            <h1>{{ $category->title }} List</h1>
        </div>
        <a href="{{ route('admin.item.category.post.create', ['id' => Session::get('item_category_id')]) }}" class="btn btn-primary icon-btn text-white"><i class="fa fa-plus-circle"></i>Create {{ $category->title }}</a>
    </div>
@stop
...
@section('script')
    ...
    <script>
        ...
        $('#dataTableBuilder tbody').on('click', '.delete', function (e) {
            ...
            // delete item_post
            function deleteItemPost(id) {
                service.delete(`item/category/{{Session::get('item_category_id')}}/post/${id}`)
