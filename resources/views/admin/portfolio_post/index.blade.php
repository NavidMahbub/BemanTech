...
@section('site_title', 'Portfolio Post')

@section('page_title')
    <div class="app-title">
        <div>
            <h1>Portfolio Post List</h1>
        </div>
        <a href="{{ route('admin.portfolio.post.create') }}" class="btn btn-primary icon-btn text-white"><i class="fa fa-plus-circle"></i>Create PortfolioPost</a>
    </div>
@stop
        ...
        $('#dataTableBuilder tbody').on('click', '.delete', function (e) {
            ...
            swal({
                ...
                if (isConfirm) {
                    ...
                    // deleting portfolio_post
                    deletePortfolioPost(id);

                }

            });

            // delete portfolio_post
            function deletePortfolioPost(id) {
                service.delete(`portfolio/post/${id}`)
